...
class FondSavdoController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        if (Auth::user()->lavozim_id == 2 && Auth::user()->status == 'Актив') {
            ...
            return view('fond.fondsavdo', ['filial_name' => $filial_name, 'lavozim_name' => $lavozim_name, 'savdounix_id' => $savdounix_id, 'xis_oyi' => $xis_oyi, 'mijozlar' => $mijozlar, 'fond' => $fond]);
        }else{
        ...
        echo '
            <table class="table table-bordered text-center align-middle ">
                ...
                <tbody id="tab1">';
                    ...
                    $xis_oyi = xissobotoy::latest('id')->value('xis_oy');
                    $fondsavdojami = fond1::where('status', 'Актив')->where('.xis_oyi', $xis_oyi)->orderBy('.id', 'desc')->get();
                    foreach ($fondsavdojami as $fondsavdojam){
                    $id=$fondsavdojam->id;
                    $savdoid=$fondsavdojam->savdoraqami_id;
                    $savdosumma = savdo1::where('status', 'Фонд')->where('unix_id', $savdoid)->where('shartnoma_id', $id)->sum('msumma');
                    ...
                    $jClick = 0;
                    $jchegirma = 0;
                    $tulovlar = tulovlar1::where('tulovturi', 'Фонд')->where('shartnomaid', $id)->where('status', 'Актив')->get();
                    foreach ($tulovlar as $tulovla) {
                        ...
                        <tr class="align-middle">
                            ...
                            <td>' . date('d.m.Y', strtotime($fondsavdojam->kun)) .'</td>
                            <td>' . $fondsavdojam->fond->pastav_name .'</td>
                            ...
                            ';
                            $tekshirtovar = savdo1::where('status', 'Фонд')->where('unix_id', $savdoid)->where('shartnoma_id', $id)->where('shtrix_kod', 0)->count();
                            if ($tekshirtovar>0){
                                echo'
                                    <td>
                                        <button id="kivitpechat" data-id="' . $fondsavdojam->id .'" data-fio="' . $fondsavdojam->mijozlar->last_name . ' ' . $fondsavdojam->mijozlar->first_name . ' ' . $fondsavdojam->mijozlar->middle_name .'"
                                        ...
                                        <button id="fondsavdoudalit"
                                            class="btn btn-outline-danger btn-sm me-2"
                                            data-id="' . $fondsavdojam->id .'"
                                            data-kun="' . $fondsavdojam->kun .'"
                                            data-savdoid="' . $fondsavdojam->savdoraqami_id .'"><i
    ...
    public function store(Request $request)
    {
        ...
        $messages = [
            ...
            'fond.required' => 'Фондни танланг.',
            'mijoz.required' => 'Мижозни танланг.',
            'savdounix_id.required' => 'Савдо-раками танланг.',
            'muddat.required' => 'Шартнома муддатини танланг.',
            'naqd.required' => 'Тўловни киритинг.',
            'plastik.required' => 'Тўловни киритинг.',
            'click.required' => 'Тўловни киритинг.',
        ...
        if ($validator->fails()) {
            ...
            if($msumma>0){
                try {
                    ...
                    $fond1->savdoraqami_id = $request->savdounix_id;
                    $fond1->muddat = $request->muddat;
                    $fond1->xis_oyi = $xis_oyi;
                    $fond1->fond_id = $request->fond;
                    $fond1->user_id = Auth::user()->id;
                    $fond1->save();
                    $insid = $fond1->id;
                    ...
                    if ($fond1 && $savdo1Updated && $tulovlar) {
                        DB::commit();
    ...
    {
        $savdounix_id = savdo1::select('unix_id')->where('status', 'Актив')->orderBy('unix_id', 'desc')->groupBy('unix_id')->get();
                echo '<option value="">Савдо рақами...</option>';
            foreach ($savdounix_id as $savdounix_i){
                echo '<option value="'.$savdounix_i->unix_id .'">'. $savdounix_i->unix_id .'</option>';
            }
        return;
    }
...

    {
        ...
        $naqdsavdojami = fond1::where('id', $id)->where('status', 'Актив')->get();
        foreach ($naqdsavdojami as $naqdsavdojam){
        }

        $tulovlar = tulovlar1::where('status', 'Актив')->where('shartnomaid', $id)->where('tulovturi', 'Фонд')->get();
       ...
       foreach($tulovlar as $tulovla){
                ...
                <table class="table-sm table-hover" style="font-size: 12px; text-align: center; width:100%; border: 1px solid black;">
                    <tbody>
                        ...
                        <tr class="align-middle text-muted">
                            ...
                            <td style="border: 1px solid black; border-collapse: collapse; padding: 5px;" colspan="5"><b style="text-transform: uppercase;">'.$naqdsavdojam->mijozlar->last_name.' '.$naqdsavdojam->mijozlar->first_name.' '.$naqdsavdojam->mijozlar->middle_name.'</b></td>
                        </tr>
                        <tr class="align-middle text-muted">
                            ...
                            <td style="border: 1px solid black; border-collapse: collapse; padding: 5px;">Фонд №:</td>
                        ...
                        <tr class="align-middle text-muted">
                            <td style="border: 1px solid black; border-collapse: collapse; padding: 5px; text-transform: uppercase;" colspan="7"><b>'.num2str($tulovla->umumiysumma).'</b></td>
                        </tr>
                    </tbody>
                </table>
            ';
        ...
        <br>
            <table class="table-sm table-hover" style="font-size: 12px; text-align: center; width:100%; border: 1px solid black;">
                ...
                $savdomodel = savdo1::where('status', 'Фонд')->where('shartnoma_id', $id)->get();
                $i = 0;
                $jami = 0;
                ...
                echo '
                    <tr class="text-center align-middle fw-bold">
                        ...
                        <td style="border: 1px solid black; border-collapse: collapse; padding: 5px;"></td>
                        <td style="border: 1px solid black; border-collapse: collapse; padding: 5px;"></td>
                        <td style="border: 1px solid black; border-collapse: collapse; padding: 5px;">' . number_format($i, 0, ",", " ") . '</td>
    ...
    {
        $tekshirktovar1 = ktovar1::where('status', 'Фонд')->where('shatnomaid', $id)->count();
        $tekshirsavdo1 = savdo1::where('status', 'Фонд')->where('unix_id', $request->savdoid)->where('shartnoma_id', $id)->where('shtrix_kod','>', 0)->count();
        if ($tekshirktovar1==0 && $tekshirsavdo1==0){
            $Readfond1 = fond1::where('id', $id)->where('status', 'Актив')->first();
            ...
            if($Readfond1->id>0){
                if($FondKun==$BugungiKun){
                    try {
                        ...
                        $tulovUpdated = tulovlar1::where('tulovturi', 'Фонд')
                        ...
                        $fondUpdated = fond1::where('id', $id)
                            ...
                            if ($savdUpdated && $tulovUpdated && $fondUpdated) {
                            DB::commit();
                    ...
                    return response()->json(['message' => 'Хатолик: ИД '.$id.' даги Фонд савдони ўчириш учун админга мурожат қилинг.'], 200);
                }
            }else{
