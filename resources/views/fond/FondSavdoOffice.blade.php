...
@section('content')
    ...
    <div class="content-body">
        <div class="page-titles" style="justify-content:center !important">
            <ol class="breadcrumb">
                <li>
                    <h5 class="heading mb-0 text-primary text-center text-uppercase fw-bold">Фонд савдоларга ўзгартириш
                        киритиш бўлими
                    </h5>
                </li>
            </ol>
        ...
        <script>
            function tabyuklash() {
                ...
                if (id > 0) {
                    $.ajax({
                        url: "{{ route('FondSavdoOffice.index') }}/" + id,
                        method: "GET",
                        data: {
                            filial: id,
                            _token: csrf
            ...
            $(document).on('click', '#kivitpechat', function() {
                ...
                var csrf = document.querySelector('meta[name="csrf-token"]').content;
                $.ajax({
                    url: "{{ route('FondSavdoOffice.store') }}",
                    method: "POST",
                    data: {
                        _token: csrf,
                        id: id,
